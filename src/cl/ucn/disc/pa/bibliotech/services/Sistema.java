...
public final class Sistema {
    ...
    public void iniciarSession(final int numeroDeSocio , final String contrasenia) {
        ...
        if(!contraseniaSocio.equals (contrasenia)){
            throw new IllegalArgumentException ("La contraseña ingresada no es valida");
        }
    ...
    public void realizarPrestamoLibro(final String isbn) throws IOException {
        ...
        if (libro == null|| libro.getCantLibro ()==0) {
    ...
    public String obtegerCatalogoLibros() {
        ...
        for (Libro libro : this.libros) {
            ...
            sb.append ("Calificacion : ").append (libro.getPromCalificacion ()).append ("\n");
            sb.append ("Cantidad Disp : ").append (libro.getCantLibro ()).append ("\n");
            sb.append ("\n");
        }
    ...
    public void calificarLibro(String isbn , int calificacion) throws IOException {
        ...
        double calificacionFinal= totalCalif/cont;
        libro.setContCalificacion (cont);
        libro.setCalificacion (totalCalif);
        libro.setPromCalificacion (calificacionFinal);
        guardarInformacion ();
    ...
    private Socio BuscarSocio(final String numeroDeSocio) {
        ...
        for (Socio socio : this.socios) {
            // si lo encontre, retorno el libro.
            String numeroSocio= String.valueOf (socio.getNumeroDeSocio ());
            if (numeroSocio.equals(numeroDeSocio)); { //libro.getIsbn ().equals (isbn)
                return socio;
            }
        }
        // no lo encontre, retorno null.
